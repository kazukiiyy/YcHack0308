...
class BridgeMonitor extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Icon(Icons.swap_horiz_rounded, size: 18, color: AppColors.accent),
  ...
  Widget _buildHeader(BuildContext context) {
    const style = TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.textSecondary, letterSpacing: 0.5);
    return Padding(
  ...
  Widget _buildRow(BuildContext context, BridgeTransaction tx) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          SizedBox(width: 120, child: Text(timeStr, style: monoStyle(size: 12, color: AppColors.textSecondary))),
          SizedBox(
            width: 140,
            child: Text(tx.publicTxHash, style: monoStyle(size: 12, color: AppColors.accent)),
          ),
          SizedBox(
            width: 140,
            child: Text(amountStr, style: monoStyle(size: 12, weight: FontWeight.w600)),
          ),
      ...
      child: Row(
        ...
        children: [
          Container(
            width: 6,
            height: 6,
            decoration: const BoxDecoration(shape: BoxShape.circle, color: AppColors.emerald),
          ),
          const SizedBox(width: 4),
          Text('LIVE', style: monoStyle(size: 10, color: AppColors.emerald, weight: FontWeight.w700)),
        ],
      ),
    );
  }
...
class _BridgeStatusIndicator extends StatelessWidget {
  ...
  Widget _stepChip(String label, int step, int activeStep) {
    ...
    if (isCurrent && step == 2) {
      ...
      fg = AppColors.textSecondary.withValues(alpha: 0.4);
    }
    ...
    return Container(
      ...
      child: Text(label, style: monoStyle(size: 10, color: fg, weight: isCurrent ? FontWeight.w600 : FontWeight.w400)),
    );
  }
